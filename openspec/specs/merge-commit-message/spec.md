...
Auto-populate the commit dialog's message editor with Git's prepared merge commit message (or a synthesized fallback) whenever the repository is in the merging state, so the user does not have to retype `Merge branch 'X' into Y` after resolving a conflict. User-authored edits must survive subsequent UI refreshes.
